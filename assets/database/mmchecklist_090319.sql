...
/* Procedure structure for procedure `count_per_category` */

/*!50003 DROP PROCEDURE IF EXISTS  `count_per_category` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `count_per_category`(in _region_id int, in _dept_id int)
BEGIN
	SELECT d.`id`, d.`category_name`,
	...
	AND b.`year` = YEAR(NOW())
	GROUP BY d.`id`, d.`category_name`;
	END */$$
DELIMITER ;

/* Procedure structure for procedure `count_per_item` */

/*!50003 DROP PROCEDURE IF EXISTS  `count_per_item` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `count_per_item`(in _region_id int, in _dept_id int)
BEGIN
	
	SELECT a.`id` as item_id, a.`item_name`, d.`id` as category_id, d.`category_name`,
	SUM(IF(c.`choice`='ok', 1, 0)) AS count_ok,
	SUM(IF(c.`choice`='no', 1, 0)) AS count_no,
	SUM(IF(c.`choice`='na', 1, 0)) AS count_na
	FROM item_checklist AS a
	...
	INNER JOIN `checklist_detail` AS c ON a.`id` = c.`item_id`
	INNER JOIN `category` AS d ON a.`category_id` = d.`id`
	WHERE b.`month` = MONTH(NOW())
	AND b.`year` = YEAR(NOW())
	AND a.`region_id` = _region_id
	AND a.`dept_id` = _dept_id
...
/* Procedure structure for procedure `count_per_monthyear` */

/*!50003 DROP PROCEDURE IF EXISTS  `count_per_monthyear` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `count_per_monthyear`(in _user_id int)
BEGIN
...
/* Procedure structure for procedure `count_per_store` */

/*!50003 DROP PROCEDURE IF EXISTS  `count_per_store` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `count_per_store`(in _user_id int)
BEGIN
	SELECT a.`store`, f.store_name, a.`month`, a.`year`, 
	SUM(IF(b.`choice`='ok', 1, 0)) AS count_ok,
	SUM(IF(b.`choice`='no', 1, 0)) AS count_no,
	SUM(IF(b.`choice`='na', 1, 0)) AS count_na
	FROM `checklist_head` AS a
	...
	INNER JOIN minimart.`department` AS d ON a.`dept_id` = d.`id` AND c.`id` = d.`region_id`
	INNER JOIN `minimart`.`user_store` AS e ON a.`store` = e.`store_id`
	inner join `minimart`.`store` as f on a.store = f.id and c.id = f.region_id
	WHERE a.`month` = MONTH(NOW())
	AND a.`year` = YEAR(NOW())
	AND e.`user_id` = _user_id
	GROUP BY a.`store`, f.store_name, a.`month`, a.`year`;
END */$$
DELIMITER ;
